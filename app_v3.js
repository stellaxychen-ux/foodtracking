...
const PCare = (() => {
  ...
  function renderEvidence(){
    ...
    const entries = state.tcmDB?.entries || [];
    view.innerHTML = `
      <div class="card p-5">
...
/* ===== Manual Add (v3-compatible additive patch) =====
   - Does NOT change existing UI/logic
   - Reuses existing addFood(name, cal, prot) if present
*/
function bindManualAdd(){
    ...
    addBtn.addEventListener('click', manualAddFromForm);
    clearBtn.addEventListener('click', manualClearForm);

    const nameEl = document.getElementById('manualName');
    if(nameEl){
        nameEl.addEventListener('keydown', (e)=>{
            if(e.key === 'Enter') manualAddFromForm();
        });
    }
}

function manualAddFromForm(){
    ...
    if(!name){
        alert('Please enter a food name / 请填写食物名称');
        return;
    }

    // Prefer local addFood if defined in this script
    if(typeof addFood === 'function'){
        addFood(name, cal, prot);
    } else if(window.PCare && typeof window.PCare.addFood === 'function'){
        window.PCare.addFood(name, cal, prot);
    } else if(typeof window.addFood === 'function'){
        window.addFood(name, cal, prot);
    } else {
        alert('Manual add hook failed: addFood() not found.');
        return;
    }
...
function manualClearForm(){
    ['manualName','manualCal','manualProt'].forEach(id=>{
